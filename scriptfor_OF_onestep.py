...
d = 1.0/10.0  #grid resolution
celln=1 # flat wall cell number 

h=0.04
res = d*h
n = 1 # Number of points to add in y direction coupling
...
for i in range (0 , Nx ) : 
    if i <= (Nx/2)-2.5/d +1 +6 or i >= (Nx/2)+2.5/d -3-6: 
        st = int (Ny/2)+1 
        print(f"Dataset st={((Nx/2)-2.5/d +1 +6)}, NY= {Ny} ")
        for j in range (0 , Ny ) :
                for k in range (0 , Nz ) :
